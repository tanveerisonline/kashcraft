...
export class NotificationService {
  ...
  async sendOrderConfirmation(userId: string, order: Order): Promise<void> {
    ...
    try {
      ...
      this.logger.info(`Order confirmation email sent to ${userEmail} for order ${order.orderNumber}`)
    } catch (error: any) {
  ...
  async sendShippingUpdate(userId: string, order: Order, tracking: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to send shipping update email for order ${order.orderNumber}`, error)
    }
  }
