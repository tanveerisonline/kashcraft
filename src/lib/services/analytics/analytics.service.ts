...
export class AnalyticsService {
  ...
  async trackProductView(productId: string, userId?: string): Promise<void> {
    this.logger.info(`Product view tracked: ProductId=${productId}, UserId=${userId || 'anonymous'}`);
    await this.prisma.analyticsEvent.create({
      data: {
  ...
  async trackAddToCart(productId: string, userId?: string): Promise<void> {
    this.logger.info(`Add to cart tracked: ProductId=${productId}, UserId=${userId || 'anonymous'}`);
    await this.prisma.analyticsEvent.create({
      data: {
  ...
  async getDashboardMetrics(startDate: Date, endDate: Date): Promise<DashboardMetrics> {
    this.logger.info(`Fetching dashboard metrics from ${startDate.toISOString()} to ${endDate.toISOString()}`);
