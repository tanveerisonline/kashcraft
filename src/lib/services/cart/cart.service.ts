  ...
  async validateCart(sessionId: string): Promise<CartValidation> {
    ...
    for (const item of cart.items) {
      ...
      if (product.stockQuantity < item.quantity) {
        isValid = false
        errors.push(`Not enough stock for product ${product.name}. Available: ${product.stockQuantity}, Requested: ${item.quantity}.`)
      }
      if (product.price.toNumber() !== item.price) {
        // Price has changed since item was added to cart
        isValid = false
        errors.push(`Price for product ${product.name} has changed from ${item.price} to ${product.price.toNumber()}.`)
